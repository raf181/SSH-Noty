...
func NewParser() *Parser {
	return &Parser{
		...
		// Disconnected/closed before auth completes (treat as failure)
		reConnClosed: regexp.MustCompile(`^(?:Disconnected from|Connection closed by) (?:invalid user )?(?:authenticating user )?(\S+) ([\da-fA-F:\.]+) port (\d+) \[preauth\]`),
